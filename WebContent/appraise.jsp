	...
	<%
		...
		if(sectionform[0][9].equals("Y"))
		{
			...
			<%for(int t=0;t<slen;t++)
			{
				...
				%>
				<tr><td><label><b><%=sectionform[t][2] %></b></label></td></tr>
				<tr>
					
						<td><label><font size="3">Your comments</font></label>
						<input name=<%=selfrate %> class="self_rating" id="<%=j %>t_self<%=t %>" maxlength="1" size="1" style="float:right" value=<%=self_rate %>></td>
						<td><label><font size="3">Supervisor's comments</font></label>
						<input name=<%=superrate %> class="super_rating" id="<%=j %>t_super<%=t %>" maxlength="1" size="1" style="float:right" value=<%=super_rate %>></td>
						<td><label><font size="3">Reviewer's comments</font></label>
						<input name=<%=revrate %> class="rev_rating" id="<%=j %>t_rev<%=t %>" maxlength="1" size="1" style="float:right" value=<%=rev_rate %>></td>
						
				</tr>
				<tr>
					<td><textarea name="<%=selfreview %>" id="<%=j %>ta_self<%=t %>" class="self_review" rows="4" cols="40" ><%=self_comment %></textarea></td>
					<td><textarea name="<%=superreview %>" id="<%=j %>ta_super<%=t %>" class="super_review" rows="4" cols="40" ><%=super_comment %></textarea></td>
					<td><textarea name="<%=revreview %>" id="<%=j %>ta_rev<%=t %>" class="rev_review" rows="4" cols="40" ><%=rev_comment %></textarea></td>
		...
		%></table><%	
			...
			for(int t=0;t<sectionform.length;t++)
			{
				...
				String revreview="revrev"+j+t;
				%>
				<tr><td><label id="<%=sectionform[t][2]%>" ><b><%=sectionform[t][2] %></b></label></td></tr>
				<tr><td style="text-align:center"><label>What you had to say..?</label></td></tr>
		  		<tr><td><textarea id="<%=j %>ta_self<%=t %>" class="self_review" rows="5" cols="100"  name="<%=selfreview%>"  ><%=self_comment %></textarea></td></tr>
		  		<tr><td style="text-align:center"><label >To be filled by your supervisor..</label></td></tr>
		  		<tr><td><textarea id="<%=j %>ta_super<%=t %>" class="super_review" rows="5" cols="100"  name="<%=superreview%>"  ><%=super_comment %></textarea></td></tr>
				<tr><td style="text-align:center"><label>To be filled by your reviewer..</label></td></tr>
				<tr><td><textarea id="<%=j %>ta_rev<%=t %>" class="rev_review" rows="5" cols="100"  name="<%=revreview%>"  ><%=rev_comment %></textarea></td></tr>
		  		<%
				}
			%></table><%
			}
